  ...
  const handleContinue = async () => {
    if (!lastExamPercentage.trim()) {
      showToast({
        ...
        message: 'Please enter your last exam percentage',
      });
      return;
    }
    ...
    try {
      // Convert marks history to proper format
      const formattedMarksHistory = Object.entries(marksHistory)
        .filter(([_, marks]) => marks.trim() !== '')
        .map(([subject, marks]) => ({
          subject,
          marks: parseFloat(marks) || 0,
          totalMarks: 100, // Assuming out of 100 for simplicity
          examDate: new Date().toISOString(),
        }));

      await updateProfile({
        lastExamPercentage: percentage,
        marksHistory: formattedMarksHistory,
      });
      
      await updateOnboardingStep(2);
      router.push('/(onboarding)/background-assessment');
    } catch (error) {
      showToast({
        type: 'error',
        title: 'Error',
