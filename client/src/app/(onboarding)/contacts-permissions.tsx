  ...
  const handleContinue = async () => {
    ...
    try {
      await updateProfile({
        contacts: {
          classTeacher: {
            name: classTeacher.name.trim(),
            phone: classTeacher.phone.trim(),
            email: classTeacher.email.trim() || undefined,
          },
          parent: {
        ...
        preferences: {
          diagnosticType: 'quick', // Default to quick, will be set in next screen
          notifications: permissions.notifications,
          dataUsage: permissions.dataUsage,
          messaging: permissions.messaging,
        },
      });
      
      await updateOnboardingStep(4);
      router.push('/(onboarding)/diagnostic-preference');
    } catch (error) {
      showToast({
        type: 'error',
        title: 'Error',
